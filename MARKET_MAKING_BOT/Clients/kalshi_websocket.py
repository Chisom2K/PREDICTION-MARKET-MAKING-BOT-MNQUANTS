...
class KalshiClient():
    ...
    async def connect(self): 
        ...
        self._ws = await websockets.connect(self.ws_base_, additional_headers=self.extra_headers_, ping_interval=5, ping_timeout=5)
        logging.info("Connected to websocket!")

    async def subscribe(self): 
        try: 
            while self.yf_client.get_price() == -1: 
                await asyncio.sleep(0.5)
            # Calculates the market price based on the current S&P 500 value, aligning with Kalshi's predefined market intervals.
            market_price = 50 * int(self.yf_client.get_price() / 50) + 25
